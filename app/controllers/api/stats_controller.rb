  ...
  class StatsController < ApplicationController
    def index
      ...
      render json: {
        ...
        top_traders: top_traders.map do |r|
          { name: r.name, trader_type: r.trader_type, trade_count: r.trade_count.to_i, total_value: r.total_value.to_f }
        end,
        top_stocks: top_stocks.map do |r|
          { ticker: r.ticker, company_name: r.company_name, trade_count: r.trade_count.to_i,
            total_value: r.total_value.to_f }
        end,
        recent_activity: recent_trades.as_json(include: { trader: { only: %i[id name trader_type avatar_url] } })
      }
    end
    ...
    def by_type
      ...
      render json: rows.map { |r|
        { trader_type: r.trader_type, trade_count: r.trade_count.to_i, total_value: r.total_value.to_f,
          avg_value: r.avg_value.to_f }
      }
    end
    ...
    def timeline
      ...
      render json: rows.map { |r|
        { date: r.date.to_date.iso8601, trade_count: r.trade_count.to_i, total_value: r.total_value.to_f }
      }
    end
